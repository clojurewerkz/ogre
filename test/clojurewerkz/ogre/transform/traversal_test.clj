...
(deftest test-transform-step
  ...
  (testing "test_g_v1_out()"
    (let [vs (q/query (v/find-by-id (g/new-tinkergraph) 1)
                      q/-->
                      q/into-vec!)]
      (is (= #{"vadas" "josh" "lop"}
             (u/get-names-set vs)))))
  ...
  (testing "test_g_v2_in()"
    (let [vs (q/query (v/find-by-id (g/new-tinkergraph) 2)
                      q/<--
                      q/into-vec!)]
      (is (= #{"marko"}
             (u/get-names-set vs)))))
  ...
  (testing "test_g_v4_both()"
    (let [vs (q/query (v/find-by-id (g/new-tinkergraph) 4)
                      q/<->
                      q/into-vec!)]
      (is (= #{"marko" "ripple" "lop"}
             (u/get-names-set vs)))))
  ...
  (testing "test_g_v1_outE()"
    (let [es (q/query (v/find-by-id (g/new-tinkergraph) 1)
                      q/-E>
                      q/into-vec!)]
      (is (= 3 (count es)))
      (is (= #{:created :knows}
  ...
  (testing "test_g_v2_outE()"
    (let [es (q/query (v/find-by-id (g/new-tinkergraph) 2)
                      q/<E-
                      q/into-vec!)]
      (is (= 1 (count es)))
      (is (= #{:knows}
  ...
  (testing "test_g_v4_bothE()"
    (let [es (q/query (v/find-by-id (g/new-tinkergraph) 4)
                      q/<E>
                      q/into-vec!)]
      (is (= 3 (count es)))
      (is (= #{:knows :created}
